...
function mostrarTecnicos() {
 ...
 .then(d => {
  ...
  if (!d || !d.data || d.data.length === 0) {
   listaTecnicos.innerHTML = "Sin técnicos";
   return;
  }
...
function mostrarProyectos() {
 ...
 .then(d => {
  ...
  if (!d || !d.data || d.data.length === 0) {
   listaProyectos.innerHTML = "Sin proyectos";
   return;
  }
...
proyecto.addEventListener("change", () => {
 const selected = proyecto.options[proyecto.selectedIndex];
 sitio.value = selected.getAttribute("data-sitio");
});
 ...
 .then(d => {
  ...
  d.data.forEach(i => {
   ...
   <div>
    ...
    <button onclick='descargarInforme(${JSON.stringify(i)})'>📄</button>
